...
parser.add_argument(
    "--k_back",
    type=int,
    help="Number of predictions made per target for retrosynthesis",
    default=10,
)
parser.add_argument(
    "--k_forward",
    type=int,
    help="Number of predictions made per reactant set for forward synthesis",
    default=2,
)
parser.add_argument(
    "--fwd_model",
    type=str,
    help='Name of forward model to use. Choose from ["gcn" and "lct"]',
    default="gcn",
)
parser.add_argument(
    "--invsmiles",
    type=int,
    help="Number of predictions check for invalid smiles per target",
    default=20,
)
parser.add_argument(
    "--dup", type=int, help="Number of predictions check for duplicates", default=20
)
parser.add_argument(
    "--stereo",
    type=str2bool,
    help="Whether to remove stereochemistry for fwd model: [True for gcn, False for lct]",
    default=True,
)
parser.add_argument(
    "--check", type=str2bool, help="Remove invalid smiles from files", default=True
)
parser.add_argument(
...
parser.add_argument(
    "--quick_eval",
    type=str2bool,
    help="Whether to evaluate results on the fly",
    default=True,
)
parser.add_argument(
...
def constructor():
    ...
    for retro_alg in algorithms:
        ...
        alg_instance = alg(
            retro_alg, args.check, info["skip"], args.stereo, args.data_path
        )
        ...
        if args.quick_eval:
            ...
            for i, metric in enumerate(eval_metrics):
                print(
                    f"Top-{metric.k} {metric.__name__}: {np.round(summary_stats[i], 2)}\n"
                )
            print("-" * num_chars)
