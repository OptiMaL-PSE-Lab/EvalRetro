...
def plot_rt(algorithms=algorithms, fig_dir=fig_dir, results_dir=results_dir):
    ...
    for retro_alg in algorithms:
        try:
            rt_data = pd.read_csv(
                os.path.join(results_dir, retro_alg.lower(), "Round-trip.csv")
            )
            ...
            rt_data["bin"] = pd.cut(
                rt_data["acc_mean"], bins=bins, include_lowest=False
            )
...
def plot_sc(algorithms=algorithms, fig_dir=fig_dir, results_dir=results_dir):
    ...
    for retro_alg in algorithms:
        try:
            sc_data = pd.read_csv(
                os.path.join(results_dir, retro_alg.lower(), "SCScore.csv")
            )
    ...
    for i, (name, sc) in enumerate(alg_sc.items()):
        ...
        try:
            ...
            ax.axvline(
                x=x, color="r", linestyle="-", linewidth=1, label=f"Mean={x:.2f}"
            )
    ...
    left_1, bottom_1, left_2, bottom_2, width, heigth = [
        0.6,
        0.5,
        0.46,
        0.18,
    ...
    for retro_alg in algorithms:
        try:
            div_data = pd.read_csv(
                os.path.join(results_dir, retro_alg.lower(), "Diversity.csv")
            )
            alg_div_mean[f"{retro_alg}"] = div_data["No_classes"].mean()
            bins = list(range(0, 9))
            div_data["bin"] = pd.cut(
                div_data["No_classes"], bins=bins, include_lowest=True
            )
...
def plot_div_dist(algorithms=algorithms, fig_dir=fig_dir, results_dir=results_dir):
    ...
    for i, (_, div) in enumerate(alg_div_dist.items()):
        ...
        try:
            ...
            new_bars = ax.bar(
                "Ground-truth", rxn_freq[i], width=1, color="orange", edgecolor="black"
            )
...
def plot_dup(algorithms=algorithms, fig_dir=fig_dir, results_dir=results_dir):
    ...
    for retro_alg in algorithms:
        try:
            dup_data = pd.read_csv(
                os.path.join(results_dir, retro_alg.lower(), "Duplicates.csv")
            )
    ...
    for i, (name, acc) in enumerate(alg_dup.items()):
        ...
        try:
            ax = axs[row_idx, col_idx]
            ax.bar(
                x=acc.bin, height=acc.dup, edgecolor="black", align="edge", width=0.1
            )
            ax.set_title(name, fontsize=16)
...
def table_round_trip(algorithms=algorithms, results_dir=results_dir):
    ...
    for retro_alg in algorithms:
        try:
            ...
            rt_acc_mean = [
                rt_data_mean[f"{met}_top_{k}"] for met in metrics for k in top_k
            ]
    ...
    for retro_alg in algorithms:
        ...
        try:
            inv_data = pd.read_csv(
                os.path.join(results_dir, retro_alg.lower(), "InvSmiles.csv"),
                names=["Top-k", "value"],
                header=0,
                index_col=False,
            )
            ...
            values *= 100
            alg_inv[f"{retro_alg}"] = pickle.load(
                open(
                    os.path.join(results_dir, retro_alg.lower(), "Inv_smi.pickle"), "rb"
                )
            )
        except Exception as e:
            print(e)
            logger.error(
                f"{retro_alg} does not have InvSmiles.csv file or Inv_smi.pickle file"
            )
            continue
...
def table_topk(algorithms=algorithms, results_dir=results_dir):
    ...
    for retro_alg in algorithms:
        ...
        try:
            topk_data = pd.read_csv(
                os.path.join(results_dir, retro_alg.lower(), "Top-k.csv")
            )
...
# Make summary table
def make_sum_table(
    alg_acc_mean, alg_div_mean, alg_val_mean, alg_dup_mean, alg_sc_mean, fig_dir=fig_dir
):
    """
    Make summary table of results
    """
    df = pd.DataFrame(
        [alg_acc_mean, alg_div_mean, alg_val_mean, alg_dup_mean, alg_sc_mean]
    )
    df = df.T
