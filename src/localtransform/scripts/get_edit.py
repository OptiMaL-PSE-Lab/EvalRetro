...
def logit2pred(out, top_num):
    ...
    output_rank = [r for r in output_rank if r % class_n != 0][:top_num]
...
def logit2pred_optimized(out, top_num):
    class_n = out.size(-1)
    readout = out.view(-1)
    _, indices = torch.sort(readout, descending=True)
    mask = indices % class_n != 0
    filtered_indices = indices[mask]
    top_indices = filtered_indices[:top_num]
    selected_site = [get_id_template(a.item(), class_n) for a in top_indices]
    selected_proba = readout[top_indices].tolist()
    return selected_site, selected_proba
...
def combined_edit(virtual_bonds, real_bonds, pred_vi, pred_ri, pred_v, pred_r, top_num):
    pred_site_v, pred_score_v = logit2pred_optimized(pred_v, top_num)
    pred_site_r, pred_score_r = logit2pred_optimized(pred_r, top_num)
