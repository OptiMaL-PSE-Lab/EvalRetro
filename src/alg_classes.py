...
class Alg(ABC):
    def __init__(
        ...
        self.data_dir = os.path.join(
            data_dir, f"{self._name}", f"{self._name}_processed.csv"
        )
        self.data_dir_cleaned = os.path.join(
            data_dir, f"{self._name}", f"{self._name}_cleaned.csv"
        )
        self.file_inv_smiles = os.path.join(
            data_dir, f"{self._name}", f"{self._name}_inv_smiles.csv"
        )
    ...
    def evaluate(self, metric, args):
        ...
        try:
            ...
            results.to_csv(
                os.path.join(results_path, f"{self._name}", f"{metric.name}.csv")
            )
            ...
            logger.exception(
                f"Something went wrong with {metric.name} evaluation for {self._name} dataset."
            )
            return None
    ...
    def detect_inv_smiles(self):
        ...
        dict_inv_smiles = {
            "Index": [index for _, index in invalid_smiles],
            "Smile": [smi for smi, _ in invalid_smiles],
        }
    ...
    def canonicalize(self):
        ...
        logger.info(
            f"Canonicalizing smile strings from {self._name} dataset.\n Results are written to {self.data_dir}"
        )
        ...
        for index, (targ, react) in enumerate(
            tqdm(zip(targets, reactants), total=len(reactants))
        ):
            if isinstance(react, str):
                try:
                    ...
                    df_alg.at[index, "reactants"] = Chem.MolToSmiles(
                        mol_rct, kekuleSmiles=True, canonical=True
                    )
                    df_alg.at[index, "target"] = Chem.MolToSmiles(
                        mol_tar, kekuleSmiles=True, canonical=True
                    )
                except Exception:
                    continue
    ...
    def _quick_eval(self, results: pd.DataFrame, index_name: str) -> float:
        ...
        if index_name == "top_k":
            ...
            print_results = (
                1 - results.iat[-1, -1]
            )  # convert invalidity into validity metric
        else:
...
class LineSeparated(Alg):
    ...
    def get_data(self, metric_name: str):
        ...
        if self.check_smi and metric_name not in uncleaned_metrics:
            df_alg = pd.read_csv(
                self.data_dir_cleaned, skip_blank_lines=self._skip, index_col=0
            )
        else:
            df_alg = pd.read_csv(
                self.data_dir, skip_blank_lines=self._skip, index_col=0
            )
        ...
        for row in tqdm(rows):
            ...
            if row - old_row != 2:
                ...
                if metric_name == "scscore":
                    ...
                    yield (
                        df_new.iloc[0]["target"],
                        df_new.iloc[1:]["reactants"].tolist(),
                    )
