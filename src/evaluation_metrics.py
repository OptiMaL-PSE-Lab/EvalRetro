...
def eval_scscore(alg: object, args):
    ...
    for k, (smiles_tar, smiles_rxn) in enumerate(alg.get_data("scscore")):
        ...
        diff_scores[f"{k}_{smiles_tar}"] = calculate_diff_scscore(
            scores
        )  # Maybe add id_rxn to dictionary
...
def diversity(alg, args):
    ...
    for i, (smiles_tar, rxn) in enumerate(alg.get_data("div")):
        ...
        div_scores[f"{i}_{smiles_tar}"] = {
            "No_classes": rxn_classes,
            "Classes": classes,
            "Counts": counts,
        }
    return div_scores
...
def top_k(alg, args):
    ...
    filtered_k_dict = {
        key: value for key, value in k_dict.items() if int(key) <= k_pred_retro
    }
...
def round_trip_single(target_smile, reactant_smiles, args=[2, gcn_forward]):
    ...
    target = Chem.MolToSmiles(
        Chem.MolFromSmiles(target_smile), canonical=True, kekuleSmiles=True
    )
